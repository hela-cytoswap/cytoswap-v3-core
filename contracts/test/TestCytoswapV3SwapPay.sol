...
contract TestCytoswapV3SwapPay is ICytoswapV3SwapCallback {
    ...
    function cytoswapV3SwapCallback(
        ...
        if (pay0 > 0) {
            IERC20Minimal(ICytoswapV3Pool(msg.sender).token0()).transferFrom(sender, msg.sender, uint256(pay0));
        } else if (pay1 > 0) {
            IERC20Minimal(ICytoswapV3Pool(msg.sender).token1()).transferFrom(sender, msg.sender, uint256(pay1));
        }
    }
}
